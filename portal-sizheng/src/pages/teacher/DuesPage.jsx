...
export default function DuesPage() {
  ...
  const openCreate = () => {
    setEditing(null)
    form.resetFields()
    form.setFieldsValue({ year: dayjs().year(), month: dayjs().month() + 1, status: '未缴', amount: 0 })
    setModalOpen(true)
  }
        ...
        {!importPreview ? (
          <>
            <Upload.Dragger
              ...
              <p className="ant-upload-hint" style={{ color: '#999' }}>AI 将自动识别表结构，无需固定模板</p>
            </Upload.Dragger>
            <Alert type="info" showIcon style={{ marginBottom: 16 }}
              message={<><strong>AI 智能解析</strong> — 无需固定模板，AI 会自动识别：姓名列、年份列、月份列、金额列、状态列、日期列</>} />
