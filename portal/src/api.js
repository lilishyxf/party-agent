...
export function getOpenid() {
  let id = localStorage.getItem('portal_openid')
  if (!id) {
    id = 'u_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8)
    localStorage.setItem('portal_openid', id)
  }
  return id
}
...
export async function learnMaterial(materialId, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/learning-materials/${materialId}/learn`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function unbindMember(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/unbind`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ openid: uid }),
  })
...
export async function saveWritingDraft(openid, templateKey, userInput, aiDraft = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/writing/save`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function generateWriting(templateKey, userInput, openid = null) {
  const uid = openid || getOpenid()
  // 1. 提交任务
  const submitResp = await fetch(`${API_BASE}/api/writing/generate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  for (let i = 0; i < 120; i++) { // 最多等 4 分钟
    ...
    const pollResp = await fetch(`${API_BASE}/api/writing/task/${taskId}`)
...
export async function listWritingDrafts(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/writing/list?openid=${uid}`)
  if (!resp.ok) return []
  return resp.json()
}
...
export async function getMyTasks(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/tasks?openid=${uid}`)
  if (!resp.ok) return []
  return resp.json()
}
...
export async function signOffTask(assignmentId, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/tasks/${assignmentId}/sign`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ openid: uid }),
  })
...
export async function getMemberStats(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/member/${uid}/stats`)
  if (!resp.ok) return null
  return resp.json()
}

export async function generateQuestions(category, difficulty, count = 5) {
  const resp = await fetch(`${API_BASE}/api/questions/generate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchQuestions(category = 'charter', count = 10) {
  ...
  const resp = await fetch(`${API_BASE}/api/questions?${params}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function checkAnswer(questionId, userAnswer, openid = null, timeSpent = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/questions/check`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getQuizOverview(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/quiz/overview?openid=${uid}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function getQuizQuestions(category, openid, offset = 0, limit = 10) {
  ...
  const resp = await fetch(`${API_BASE}/api/quiz/questions?${params}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function getWrongQuestions(openid = null, category = null) {
  ...
  const resp = await fetch(`${API_BASE}/api/quiz/wrong?${params}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function getExamDetail(examId, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/exams/${examId}?openid=${uid}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function submitExam(examId, answers, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/exams/${examId}/submit`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getMyExamResult(examId, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/exams/${examId}/my-result?openid=${uid}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function getExamHistory(openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/exams/my-history?openid=${uid}`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function getLearningRecords(openid = null, page = 1) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/member/${uid}/learning-records?page=${page}&page_size=20`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}

export async function fetchPortalNotices(page = 1) {
  const resp = await fetch(`${API_BASE}/api/notices?status=published&page=${page}&page_size=20`)
  if (!resp.ok) throw new Error(`API error: ${resp.status}`)
  return resp.json()
}
...
export async function fetchNotes(materialId, openid = null) {
  const uid = openid || getOpenid()
  const resp = await fetch(`${API_BASE}/api/notes?material_id=${materialId}&openid=${uid}`)
  if (!resp.ok) return []
  return resp.json()
}
