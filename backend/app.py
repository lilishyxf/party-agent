...
app = FastAPI(title="Party Affairs Agent API", lifespan=lifespan)

# Serve uploaded files
_materials_dir = "/opt/party-agent/frontends/admin/media/materials"
if os.path.isdir(_materials_dir):
    app.mount("/media/materials", StaticFiles(directory=_materials_dir), name="media_materials")
...
# Mount sizheng portal API at /api/sizheng/
from app_sizheng import sizheng_app
app.mount("/api/sizheng", sizheng_app)
...
@app.put("/api/party-groups/{group_id}")
...
@app.delete("/api/party-groups/{group_id}")
...
@app.get("/api/party-groups/{group_id}/members")
...
@app.get("/api/development/{member_id}/history")
...
@app.post("/api/development/advance")
...
@app.get("/api/member/{openid}/stats")
...
@app.get("/api/member/{openid}/learning-records")
...
@app.put("/api/learning-materials/{material_id}")
...
@app.delete("/api/learning-materials/{material_id}")
...
@app.post("/api/learning-materials/upload")
...
@app.post("/api/learning-materials/{material_id}/learn")
...
@app.get("/api/writing/task/{task_id}")
...
@app.post("/api/tasks/{assignment_id}/sign")
...
@app.post("/api/admin/tasks/{task_id}/assign")
...
@app.get("/api/exams/{exam_id}/my-result")
...
@app.post("/api/topics/{topic_id}/items")
...
@app.delete("/api/topics/items/{item_id}")
...
@app.get("/api/learning-records/export")
...
@app.post("/api/questions/pre-generate")
...
@app.post("/api/questions/pre-generate-all")
...
@app.get("/api/promotion/images/albums")
...
@app.post("/api/promotion/images/upload")
...
@app.delete("/api/promotion/images/delete")
...
async def import_dues_preview(
    ...
    try:
        ...
        return {"status": "error", "message": "文件格式不支持，请上传 .xlsx 格式的 Excel 文件（不支持 .xls 旧格式和 CSV）"}
    ws = wb.active
...
请返回 JSON，将原始表头映射到标准字段。标准字段名: member_name(姓名), year(年份), month(季度), amount(金额), status(状态), paid_date(缴费日期), notes(备注)。

识别规则:
- 状态列的值如"已缴/未缴/减免" → status
- 数字且包含"年"或值为2020-2030范围 → year
- 数字且值为1-4范围，或文本含Q1/Q2/Q3/Q4、"季度" → month(季度)
- 金额数字（带"元"或小数）→ amount
- 日期格式 → paid_date
- 长的文本 → notes
...
@app.post("/api/dues/import/confirm")
...
def auto_detect_dues_columns(headers: list[str]) -> dict:
    ...
    KEYWORDS = {
        ...
        "year": ["年份", "年", "年度"],
        "month": ["季度", "季", "Q1", "Q2", "Q3", "Q4", "q1", "q2", "q3", "q4", "月份", "月"],
...
@app.get("/api/operation-logs/unread-count")
...
@app.post("/api/operation-logs/{log_id}/read")
...
@app.post("/api/operation-logs/read-all")
...
@app.delete("/api/system-configs/{config_id}")
...
@app.get("/api/supervision/discipline-study")
...
@app.post("/api/supervision/discipline-study")
...
@app.put("/api/supervision/discipline-study/{did}")
...
@app.delete("/api/supervision/discipline-study/{did}")
...
@app.get("/api/supervision/warning-edu")
...
@app.post("/api/supervision/warning-edu")
...
@app.put("/api/supervision/warning-edu/{wid}")
...
@app.delete("/api/supervision/warning-edu/{wid}")
...
@app.get("/api/supervision/inspection")
...
@app.post("/api/supervision/inspection")
...
@app.put("/api/supervision/inspection/{iid}")
...
@app.delete("/api/supervision/inspection/{iid}")
...
async def dashboard_stats(year: int = None, admin: AdminUser = Depends(get_current_admin), db: AsyncSession = Depends(get_db)):
    now = datetime.now()
    this_month = now.month
    this_quarter = (now.month - 1) // 3 + 1
    this_year = now.year
    query_year = year if year else this_year
    ...
    for q in range(1, 5):
        q_total = (await db.execute(
            select(func.sum(Dues.amount)).where(Dues.year == query_year, Dues.month == q)
        )).scalar()
        q_total = round(float(q_total), 2) if q_total else 0
        q_paid = (await db.execute(
            select(func.sum(Dues.amount)).where(Dues.year == query_year, Dues.month == q, Dues.status == "已缴")
        )).scalar()
        q_paid = round(float(q_paid), 2) if q_paid else 0
        q_count = (await db.execute(
            select(func.count()).select_from(Dues).where(Dues.year == query_year, Dues.month == q)
        )).scalar() or 0
        q_paid_count = (await db.execute(
            select(func.count()).select_from(Dues).where(Dues.year == query_year, Dues.month == q, Dues.status == "已缴")
        )).scalar() or 0
        q_rate = round(q_paid_count / q_count * 100, 1) if q_count > 0 else 0
        dues_quarterly.append({"quarter": f"Q{q}", "total": q_total, "paid": q_paid, "count": q_count, "paid_count": q_paid_count, "rate": q_rate})

    # 本季度收缴率（用于顶部统计卡片）
    current_q_data = [d for d in dues_quarterly if d["quarter"] == f"Q{this_quarter}"] if query_year == this_year else []
    dues_rate = current_q_data[0]["rate"] if current_q_data else 0

    # 有党费数据的年份列表
    year_rows = (await db.execute(
        select(Dues.year).distinct().order_by(Dues.year)
    )).scalars().all()
    dues_years = sorted(list(set(year_rows))) if year_rows else [this_year]
    if this_year not in dues_years:
        dues_years.append(this_year)
    ...
    return {
        ...
        "dues_rate": dues_rate,
        "dues_quarterly": dues_quarterly,
        "dues_years": dues_years,
        "dues_year": query_year,
...
@app.put("/api/floating-members/{fid}")
...
@app.delete("/api/floating-members/{fid}")
...
@app.delete("/api/regulations/{rid}")
...
@app.delete("/api/dues-receipts/{rid}")
...
@app.delete("/api/dues-public/{did}")
